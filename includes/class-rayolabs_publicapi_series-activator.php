 ...
 */
class Rayolabs_publicapi_series_Activator {
	...
	public static function activate() {
		$options = get_option("rayolabs_publicapi_series");
	$token = $options['site_token'];
	if(!empty($token)){
		$url = site_url();
    	$url.="/index.php?my-plugin=rayolabs_publicapi_series";	
	...
	$data1 = http_build_query($whd);
			$ch = curl_init();
	$url = "https://publicapi.org.ng/api/$token/o2tvseries";
